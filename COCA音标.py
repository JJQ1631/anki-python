...
# 处理单词的函数
def process_word(note_id, expression, col, proxies, failed_words, index, total):
    ipa = get_cambridge_ipa(expression, proxies)
    if ipa:
        note = col.get_note(note_id)
        note.fields[note._field_index("音标")] = ipa
        col.update_note(note)
    else:
        failed_words.append(expression)
...
def main():
    ...
    try:
        ...
        deck_name = "COCA20000"
        notes = get_all_notes_in_deck(col, deck_name)
        ...
        with tqdm(total=total, desc="处理进度") as progress_bar:
            with concurrent.futures.ThreadPoolExecutor(max_workers=50) as executor:
                ...
                for index, note_id in enumerate(notes):
                    ...
                    futures.append(executor.submit(process_word, note_id, expression, col, proxies, failed_words, index, total))
        ...
        if failed_words:
            ...
            output_path = "COCA_pron_symbol_fail.xlsx"
